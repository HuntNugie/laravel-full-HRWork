<div class="space-y-6">
    ...
    <x-wirekit::stack gap="sm">
        <a x-on:click="window.history.back()" wire:navigate
            class="inline-flex w-fit items-center gap-2 text-sm font-medium text-black transition hover:text-[#30AFFF] cursor-pointer">
            ...
            <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    @if ($contract?->end_date)
                        <p class="mt-1 text-sm font-medium text-slate-900">
                            {{ intval(Carbon\Carbon::parse($contract?->start_date)->diffInMonths($contract?->end_date)) . ' Bulan' }}
                        </p>
                    @else
                        <p class="mt-1 text-sm font-medium text-slate-900">
                            Pegawai tetap
