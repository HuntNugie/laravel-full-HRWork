<x-wirekit::stack gap="md">
        ...
        <x-wirekit::card>
            <x-wirekit::card.body>
                <x-wirekit::stack gap="sm">
                    ...
                    <span
                        class="inline-flex w-fit items-center rounded-full px-2.5 py-1 text-xs font-medium {{ $statusClass }}">{{ str_replace('_', ' ', $task->status) }}</span>
                </x-wirekit::stack>
            </x-wirekit::card.body>
        </x-wirekit::card>
        ...
        <x-wirekit::card>
            <x-wirekit::card.body>
                <x-wirekit::stack gap="1">
                    ...
                    <span
                        class="text-sm font-semibold text-slate-900">{{ $task->team?->supervisor?->user?->name ?? '—' }}</span>
                </x-wirekit::stack>
            </x-wirekit::card.body>
        </x-wirekit::card>
    </div>
    ...
    <x-wirekit::card>
        ...
        <x-wirekit::card.body>
            <x-wirekit::stack gap="md">
                <div>
                    <span class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-400">Hasil /
                        Catatan</span>
                ...
                @if ($task->blocked_reason)
                    <div class="rounded-xl border border-amber-100 bg-amber-50 p-4">
                        <span class="mb-2 block text-xs font-semibold uppercase tracking-wide text-amber-600">Alasan
                            Blocked</span>
                ...
                <div
                    class="flex flex-col gap-1 border-t border-slate-100 pt-4 text-xs text-slate-400 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <form wire:submit="saveWork" class="space-y-5">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <x-wirekit::input type="number" min="0" max="100" wire:model="progress"
                                    name="progress" />
                                @error('progress')
                            ...
                            <div>
                                ...
                                <select wire:model.live="workStatus"
                        ...
                        <div>
                            ...
                            <textarea wire:model="result" rows="5"
                                class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-[#30AFFF] focus:ring-2 focus:ring-[#30AFFF]/20"
                                placeholder="Tuliskan hasil pekerjaan atau catatan"></textarea>
                            @error('result')
                                <span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        @if ($workStatus === 'blocked')
                            <div>
                                ...
                                <textarea wire:model="blockedReason" rows="4"
                                    class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-[#30AFFF] focus:ring-2 focus:ring-[#30AFFF]/20"
                                    placeholder="Jelaskan hambatan yang terjadi"></textarea>
                                @error('blockedReason')
                                    <span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="flex flex-wrap gap-2 border-t border-slate-100 pt-5">
                            ...
                            @if ($task->progress === 100)
                                <x-wirekit::button type="button" wire:click="submit"
            ...
            <x-wirekit::card>
                <x-wirekit::card.header>
                    <x-wirekit::stack gap="1">
                        ...
                        <p class="text-sm text-slate-500">Tinjau hasil task dan tentukan apakah perlu revisi atau dapat
                            diselesaikan.</p>
                    </x-wirekit::stack>
                </x-wirekit::card.header>
                    ...
                    <form wire:submit="review" class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea wire:model="reviewFeedback" rows="4"
                                class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-[#30AFFF] focus:ring-2 focus:ring-[#30AFFF]/20"
                                placeholder="Tuliskan feedback review"></textarea>
                ...
                @forelse ($task->reviews->sortByDesc('created_at') as $review)
                    ...
                    <div class="space-y-2 py-4 first:pt-0 last:pb-0">
                        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <div class="flex items-center gap-2">
                                <span
                                    class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium {{ $reviewClass }}">
                                    {{ ucfirst($review->decision) }}
                                </span>
                                <span
                                    class="text-sm font-medium text-slate-800">{{ $review->reviewer?->user?->name ?? '—' }}</span>
                            </div>
                            <span
                                class="text-xs text-slate-400">{{ $review->created_at?->format('d M Y H:i') }}</span>
                        </div>
