<div class="min-h-[calc(100vh-7rem)] bg-sky-50/30">
    <div class="mx-auto flex h-full max-w-6xl flex-col gap-5" x-data="{
        submitting: false,
        pendingMessage: '',
        async submitPrompt() {
        ...
        <div class="flex items-center gap-3 px-1">
            <div
                class="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#30AFFF]/10 text-[#30AFFF] ring-1 ring-[#30AFFF]/20">
        ...
        <div class="overflow-hidden rounded-3xl border border-sky-100 bg-white shadow-[0_10px_40px_rgba(48,175,255,0.08)]"
            style="
                --color-wk-bg: #ffffff;
            ...
            ">
            <x-wirekit::conversation max-height="560px" label="Percakapan HRWork AI">
                <x-wirekit::stack gap="md" class="min-h-[500px] p-4 sm:p-6">

                    <div x-show="!submitting" x-cloak x-transition.opacity.duration.150ms>
                        @if ($messages === [])
                            <div class="flex min-h-[460px] items-center justify-center">
                                <div class="w-full max-w-2xl text-center">
                                    <div
                                        class="mx-auto flex h-16 w-16 items-center justify-center rounded-3xl bg-[#30AFFF]/10 text-[#30AFFF] ring-1 ring-[#30AFFF]/10">
                                    ...
                                    <p class="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-500">
                                        ...
                                        Chat hanya ditampilkan selama halaman ini terbuka dan tidak disimpan sebagai
                                        riwayat.
                                    </p>

                                    <div class="mt-8 grid gap-3 text-left sm:grid-cols-2">
                                        @foreach (['Jelaskan fungsi HRWork AI.', 'Apa yang bisa dibantu AI dalam HRIS?', 'Bagaimana nanti AI mengakses data karyawan?', 'Apa batasan AI dalam proses HR?'] as $suggestion)
                                            <button type="button"
                    ...
                    @foreach ($messages as $index => $message)
                        @if ($message['role'] === 'user')
                            <x-wirekit::message wire:key="message-user-{{ $index }}" :author="['name' => 'Kamu']"
                                side="right">
                                {{ $message['content'] }}
                            </x-wirekit::message>
                        @else
                            <div wire:key="message-assistant-{{ $index }}">
                                <x-wirekit::assistant-message :name="'HRWork AI'"
                                    model="{{ config('ai.providers.9router.models.text.default') }}" announce="all">
                    ...
                    <template x-if="submitting && pendingMessage">
                        <div x-cloak>
                            <x-wirekit::message :author="['name' => auth()->user()->name]" side="right">
                    ...
                    <template x-if="submitting">
                        <div x-cloak>
                            <x-wirekit::message-typing author="HRWork AI" announce />
                        </div>
                    </template>
        ...
        <div class="rounded-3xl border border-sky-100 bg-sky-50/70 p-3 shadow-sm sm:p-4">
            <form x-on:submit.prevent="submitPrompt">
                <div
                    class="rounded-2xl border border-sky-100 bg-white p-2 shadow-sm transition focus-within:border-[#30AFFF]/60 focus-within:ring-2 focus-within:ring-[#30AFFF]/10">
                    <textarea x-ref="prompt" wire:model="prompt" rows="3" maxlength="5000"
                        ...
                        x-bind:disabled="submitting" wire:loading.attr="disabled"></textarea>
                    ...
                    <div class="flex items-center justify-between gap-3 px-2 pb-1">
                        ...
                        <x-wirekit::button type="submit" x-bind:disabled="submitting" wire:loading.attr="disabled"
                            size="sm" icon="paper-airplane">
                            Kirim
                        </x-wirekit::button>
                    </div>
                </div>
