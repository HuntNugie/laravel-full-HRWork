<x-wirekit::stack gap="lg">
    ...
    @php
        $attentionItems = [
            ['label' => 'User menunggu aktivasi', 'value' => $pendingUsers, 'route' => 'user.view', 'intent' => 'warning'],
            ['label' => 'Pengajuan cuti', 'value' => $pendingLeave, 'route' => 'leave.manage.view', 'intent' => 'info'],
            ['label' => 'Pengajuan sakit / izin', 'value' => $pendingAbsence, 'route' => 'absence.view', 'intent' => 'info'],
            ['label' => 'Report Work Management', 'value' => $pendingProjectReports, 'route' => 'work-management.master-projects', 'intent' => 'primary'],
            ['label' => 'Master Project siap review', 'value' => $masterReadyForReview, 'route' => 'work-management.master-projects', 'intent' => 'success'],
        ];

        $userTotalForBar = max(1, $totalUsers);
        $activeUserPercent = min(100, round(($activeUsers / $userTotalForBar) * 100));
        $pendingUserPercent = min(100, round(($pendingUsers / $userTotalForBar) * 100));
        $inactiveUserPercent = min(100, round(($inactiveUsers / $userTotalForBar) * 100));
        ...
        $payrollStatusMeta = match ($latestPayrollSummary['period']?->status) {
            ...
            default => ['label' => $latestPayrollSummary['period']?->status ? str_replace('_', ' ', $latestPayrollSummary['period']->status) : 'Belum ada periode', 'class' => 'bg-slate-100 text-slate-600'],
        };
    @endphp

    <div class="relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-950 px-6 py-7 shadow-sm sm:px-8">
        ...
        <div class="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <x-wirekit::stack gap="sm">
                <span class="text-sm font-semibold text-[#5DC4FF]">
                ...
                <div class="space-y-2">
                    ...
                    <p class="max-w-2xl text-sm leading-6 text-slate-300">
                        Pantau kondisi pengguna, organisasi, HR operations, payroll, dan Work Management dari satu halaman.
                    </p>
                </div>
            </x-wirekit::stack>
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <x-wirekit::card class="border-slate-200">
            <x-wirekit::card.body>
                <x-wirekit::stack gap="sm">
                    ...
                    <div class="flex items-center justify-between text-xs">
                        ...
                        <span class="font-semibold text-emerald-600">{{ $activeUsers }}</span>
                    </div>
                </x-wirekit::stack>
            </x-wirekit::card.body>
        </x-wirekit::card>
        ...
        <x-wirekit::card class="border-slate-200">
            <x-wirekit::card.body>
                <x-wirekit::stack gap="sm">
                    ...
                    <span class="text-xs text-slate-400">Gabungan item yang masih menunggu tindakan</span>
                </x-wirekit::stack>
            </x-wirekit::card.body>
        </x-wirekit::card>
        ...
        <x-wirekit::card class="xl:col-span-2">
            <x-wirekit::card.header>
                <x-wirekit::stack gap="1">
                    ...
                    <p class="text-sm text-slate-500">Kondisi akun dan struktur organisasi saat ini.</p>
                </x-wirekit::stack>
            </x-wirekit::card.header>
                ...
                <x-wirekit::stack gap="md">
                    <div class="flex items-end justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-400">Sudah tercatat check-in</p>
                        </div>
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        <p class="mt-1 text-[11px] leading-5 text-slate-400">
                            Angka ini juga dapat mencakup karyawan yang sedang cuti, izin, atau belum check-in.
                        </p>
                    </div>
                </x-wirekit::stack>
            </x-wirekit::card.body>
            ...
            <x-wirekit::card.header>
                <div class="flex items-start justify-between gap-4">
                    <x-wirekit::stack gap="1">
                        ...
                        <p class="text-sm text-slate-500">Antrean yang berguna untuk monitoring admin.</p>
                    </x-wirekit::stack>
                    ...
                    @foreach ($attentionItems as $item)
                        <div class="flex items-center justify-between rounded-xl border border-slate-200 px-4 py-3">
                            <div class="flex min-w-0 items-center gap-3">
                                <span class="size-2.5 shrink-0 rounded-full {{ match($item['intent']) {
                ...
                <x-wirekit::stack gap="md">
                    @if ($latestPayrollSummary['period'])
                        ...
                        <a href="{{ route('payroll.view') }}" wire:navigate>
                            <x-wirekit::button type="button" variant="outline" class="w-full">
                                Buka Payroll
                            </x-wirekit::button>
                        </a>
                    @else
                        <div class="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center">
                            ...
                            <p class="mt-1 text-xs text-slate-400">Periode payroll terbaru akan muncul di dashboard setelah dibuat.</p>
                        </div>
                    @endif
                </x-wirekit::stack>
            </x-wirekit::card.body>
        ...
        <x-wirekit::card.header>
            <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <x-wirekit::stack gap="1">
                    ...
                    <p class="text-sm text-slate-500">Ringkasan pekerjaan dan approval yang sedang berjalan.</p>
                </x-wirekit::stack>
            ...
            <x-wirekit::card.header>
                <div class="flex items-start justify-between gap-4">
                    <x-wirekit::stack gap="1">
                        ...
                        <p class="text-sm text-slate-500">Enam karyawan yang terakhir dibuat di sistem.</p>
                    </x-wirekit::stack>
                    <a href="{{ route('employee.view') }}" wire:navigate class="text-xs font-semibold text-[#168ED1] hover:underline">
                        Semua
                    </a>
                </div>
            </x-wirekit::card.header>
                        ...
                        <div class="flex items-center justify-between gap-3 rounded-xl border border-slate-200 px-4 py-3">
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 truncate text-xs text-slate-400">
                                    {{ $employee->position?->name ?? 'Tanpa position' }} · dibuat {{ $employee->created_at?->diffForHumans() }}
                                </p>
                            </div>
        ...
        <x-wirekit::card>
            <x-wirekit::card.header>
                <x-wirekit::stack gap="1">
                    ...
                    <p class="text-sm text-slate-500">Report terbaru dari Supervisor dan Manager.</p>
                </x-wirekit::stack>
            </x-wirekit::card.header>
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="truncate text-sm font-semibold text-slate-800">
                                        {{ $report->divisionProject?->name ?? 'Division Project' }}
                                    </p>
                                    <p class="mt-1 text-xs text-slate-400">
                                        {{ $reportLevelLabel }} · {{ $report->reporter?->user?->name ?? 'Tidak diketahui' }}
                                    </p>
                                </div>
                                <span class="shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold {{ $reportStatusClass }}">
    ...
    <x-wirekit::card>
        <x-wirekit::card.header>
            <x-wirekit::stack gap="1">
                ...
                <p class="text-sm text-slate-500">Shortcut untuk area administrasi yang paling sering digunakan.</p>
            </x-wirekit::stack>
        </x-wirekit::card.header>
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                @can('view-master-project')
                    <a href="{{ route('work-management.master-projects') }}" wire:navigate>
                        <x-wirekit::button type="button" class="w-full bg-[#30AFFF] text-white hover:bg-[#1599E8]">
                            Work Management
                        </x-wirekit::button>
                    </a>
                @endcan
