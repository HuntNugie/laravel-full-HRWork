<x-wirekit::stack gap="lg">
    ...
    <x-wirekit::stack gap="sm">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-sm font-medium text-[#30AFFF]">Dashboard Manager</p>
                <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-900">
                    {{ $employee->user->name }}
                </h1>
                <p class="mt-1 text-sm text-slate-500">
                    Monitoring presensi employee dalam divisi yang kamu kelola.
                </p>
            </div>
            ...
            <x-wirekit::card.body>
                <x-wirekit::stack gap="sm" class="items-center justify-center py-8 text-center">
                    <p class="text-sm font-medium text-slate-700">
                        Kamu belum menjabat sebagai manager divisi.
                    </p>
                    <p class="text-sm text-slate-500">
                        Monitoring presensi akan muncul setelah akunmu ditetapkan sebagai manager pada sebuah divisi.
                    </p>
                </x-wirekit::stack>
            </x-wirekit::card.body>
        </x-wirekit::card>
    @else
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <x-wirekit::card>
                <x-wirekit::card.body>
                    <x-wirekit::stack gap="sm">
                        <span class="text-sm font-medium text-slate-500">Division</span>
                        <p class="text-xl font-semibold text-slate-900">{{ $division->name }}</p>
                        <p class="text-xs text-slate-400">{{ $summary['teams'] }} team</p>
                    </x-wirekit::stack>
                </x-wirekit::card.body>
            </x-wirekit::card>
            ...
            <x-wirekit::card>
                <x-wirekit::card.body>
                    <x-wirekit::stack gap="sm">
                        <span class="text-sm font-medium text-slate-500">Employee</span>
                        <p class="text-3xl font-semibold text-slate-900">{{ $summary['employees'] }}</p>
                        <p class="text-xs text-slate-400">Anggota seluruh team di divisi</p>
                    </x-wirekit::stack>
                </x-wirekit::card.body>
            </x-wirekit::card>
            ...
            <x-wirekit::card>
                <x-wirekit::card.body>
                    <x-wirekit::stack gap="sm">
                        <span class="text-sm font-medium text-slate-500">Terlambat</span>
                        <p class="text-3xl font-semibold text-amber-600">{{ $summary['late'] }}</p>
                        <p class="text-xs text-slate-400">Employee terlambat hari ini</p>
                    </x-wirekit::stack>
                </x-wirekit::card.body>
            </x-wirekit::card>
        </div>
        ...
        <x-wirekit::card>
            <x-wirekit::card.header>
                <x-wirekit::stack gap="1">
                    ...
                    <p class="text-sm text-slate-500">
                        Hanya employee yang berada di team-team dalam divisi {{ $division->name }}.
                    </p>
                </x-wirekit::stack>
            </x-wirekit::card.header>
